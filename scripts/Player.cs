...
public class Player : KinematicBody, Tst.Debuggable {
    ...
    public new void QueueFree() { base.QueueFree();
}
...
public override void _PhysicsProcess(float delta) {
    ...
    if (!mIsStep && IsOnFloor()) {
        ...
        bool isPlayerCollided =
            PhysicsServer.BodyTestMotion(GetRid(), transform3d, motion, false, testMotionResult);

        if (!isPlayerCollided) {
            transform3d.origin += motion;
            motion = -stepHeight;
            isPlayerCollided = PhysicsServer.BodyTestMotion(GetRid(), transform3d, motion, false,
                                                            testMotionResult);
            if (isPlayerCollided) {
                if (testMotionResult.CollisionNormal.AngleTo(Vector3.Up) <=
                    Mathf.Deg2Rad(STEP_MAX_SLOPE_DEGREE)) {
                    ...
                    mIsStep = true;
                    GlobalTransform = Util.ChangeTFormOrigin(
                        GlobalTransform, GlobalTransform.origin + testMotionResult.MotionRemainder);
                }
            } else {
                isFalling = true;
            }
    ...
    if (mIsRealPlayer) {
        ...
        RpcUnreliable("UpdatePlayer", GlobalTransform, mHead.GlobalTransform, mBody.GlobalTransform,
                      mVelocity, mWishDir, mWishJump, mGravityVec, mSnap, mVerticalVelocity);
    }
}
...
[Master]
public void SendPlayerInput(float mouseDx, float mouseDy, float strafe, float forward, bool jump,
                            bool autoJump) {
...
public void UpdatePlayer(Transform globalTransform, Transform headTransform,
                         Transform bodyTransform, Vector3 velocity, Vector3 wishDir, bool wishJump,
                         Vector3 gravityVec, Vector3 snap, float verticalVelocity) {
